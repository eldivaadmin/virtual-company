...
from .db import init_db,add_event,recent_events,recent_real_events,create_task,finish_task,task_counts,set_presence,get_presence
...
@app.get('/api/build')
def build():return {'build':'COMPLETE-FIX-20260901-1948','root':str(BASE)}
...
def save_google_credentials(body:GoogleCredsIn):
    try:
        ...
        if not isinstance(data,dict) or not ('installed' in data or 'web' in data):
            raise ValueError('OAuth client JSONではありません')
        config.SECRETS_DIR.mkdir(parents=True,exist_ok=True)
        config.GOOGLE_CREDENTIALS.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8')
        return {'ok':True}
    except Exception as e:
        raise HTTPException(400,f'credentials.json: {e}')

_google_auth_state={}

@app.post('/api/google/connect/{email}')
def connect_google(email:str):
    allowed=set(config.google_accounts().get('mail',[]))
    cal=config.google_accounts().get('calendar')
    ...
    if not config.GOOGLE_CREDENTIALS.exists():
        raise HTTPException(400,'先にGoogle OAuth JSONを登録してください')
    import threading
    _google_auth_state[email]='starting'
    def worker():
        try:
            ok=authorize_account(email)
            _google_auth_state[email]='connected' if ok else 'failed'
        except Exception as e:
            _google_auth_state[email]='error: '+str(e)
...
def mail_summary():
    ...
    for email in config.google_accounts().get('mail',[]):
        ...
        if not svc:
            result[email]={'connected':False,'unread':None,'important_unread':None}
            continue
        q1=svc.users().messages().list(userId='me',q='in:inbox is:unread -in:spam -in:trash',maxResults=1).execute()
        q2=svc.users().messages().list(userId='me',q='in:inbox is:unread is:important -in:spam -in:trash',maxResults=1).execute()
        result[email]={'connected':True,'unread':q1.get('resultSizeEstimate',0),'important_unread':q2.get('resultSizeEstimate',0)}
    return result
...
def google_setup_page():
    return FileResponse(BASE/'web'/'google_setup.html',headers={'Cache-Control':'no-store'})
@app.get('/')
def index():
    return FileResponse(BASE/'web'/'index.html',headers={'Cache-Control':'no-store, no-cache, must-revalidate, max-age=0'})
